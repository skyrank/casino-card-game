...
function Game({ roomCode, playerRole, playerName, opponentName, onLeaveGame }) {
  ...
  async function makeAiMove() {
    ...
    if (aiHand.length === 0) {
      console.log('AI has no cards');
      return;
    }
    
    // Strategy: 1) Try captures, 2) Try builds, 3) Trail
    
    // 1. Check for captures (prioritize builds, then table cards)
    const captureMove = findBestCapture(aiHand, tableCards, builds);
    if (captureMove) {
      await executeAiCapture(captureMove);
      return;
    }
    
    // 2. Check for valid builds
    const buildMove = findBestBuild(aiHand, tableCards);
    if (buildMove) {
      await executeAiBuild(buildMove);
      return;
    }
    
    // 3. Trail lowest card
    await executeAiTrail(aiHand);
  }
  
  function findBestCapture(aiHand, tableCards, builds) {
    // Safety checks
    if (!aiHand || !Array.isArray(aiHand)) return null;
    if (!tableCards) tableCards = [];
    if (!builds) builds = [];
    
    let bestMove = null;
    let maxCardsCapture = 0;
    
    // Check each card in AI's hand
    for (let handIdx = 0; handIdx < aiHand.length; handIdx++) {
      const card = aiHand[handIdx];
      
      // Check if card can capture any builds
      const matchingBuilds = builds
        .map((build, idx) => ({ build, idx }))
        .filter(b => b.build.value === card.rank);
      
      if (matchingBuilds.length > 0) {
        const totalCards = matchingBuilds.reduce((sum, b) => sum + (b.build.cards?.length || 0), 0);
        if (totalCards > maxCardsCapture) {
          maxCardsCapture = totalCards;
          bestMove = {
            type: 'captureBuild',
            handIdx,
            buildIndices: matchingBuilds.map(b => b.idx)
          };
        }
      }
      
      // Check if card can capture table cards
      const capturableIndices = [];
      for (let i = 0; i < tableCards.length; i++) {
        if (tableCards[i]?.rank === card.rank) {
          capturableIndices.push(i);
        }
      }
      
      // Check for combinations that sum to card rank
      const combos = findCapturableCombinations(tableCards, card.rank);
      if (combos && combos.length > 0) {
        const bestCombo = combos[0]; // Take first valid combo
        const totalCards = bestCombo.cards?.length || 0;
        if (totalCards > maxCardsCapture) {
          maxCardsCapture = totalCards;
          bestMove = {
            type: 'captureTable',
            handIdx,
            tableIndices: bestCombo.indices
          };
        }
      }
    }
  ...
  function findBestBuild(aiHand, tableCards) {
    // Safety checks
    if (!aiHand || !Array.isArray(aiHand)) return null;
    if (!tableCards) tableCards = [];
    
    // Try to create a build if we have matching cards in hand
    for (let handIdx = 0; handIdx < aiHand.length; handIdx++) {
      const card = aiHand[handIdx];
      
      // Count how many of this rank we have
      const matchingCards = aiHand.filter(c => c?.rank === card.rank).length;
      
      // Need at least 2 to build safely (one to build, one to capture later)
      if (matchingCards >= 2) {
        // Find table cards that sum to this rank
        const validBuilds = findValidBuildValues(tableCards, aiHand.filter((_, idx) => idx !== handIdx));
        if (!validBuilds) continue;
        
        const matchingBuild = validBuilds.find(v => v.value === card.rank);
        
        if (matchingBuild) {
          return {
            type: 'createBuild',
            handIdx,
            tableIndices: matchingBuild.indices,
            buildValue: card.rank
          };
        }
      }
    }
  ...
  async function executeAiCapture(move) {
    const aiHand = gameState.player2Hand || [];
    const card = aiHand[move.handIdx];
    
    if (move.type === 'captureBuild') {
      // Capture builds
      if (move.buildIndices.length === 1) {
        await aiCaptureSingleBuild(move.handIdx, move.buildIndices[0]);
      } else {
        await aiCaptureMultipleBuilds(move.handIdx, move.buildIndices);
      }
    } else if (move.type === 'captureTable') {
      // Capture table cards
      await aiCaptureTableCards(move.handIdx, move.tableIndices);
    }
  }
  
  async function aiCaptureSingleBuild(handIdx, buildIdx) {
    const aiHand = gameState.player2Hand || [];
    const builds = gameState.builds || [];
    const build = builds[buildIdx];
    const playedCard = aiHand[handIdx];
    
    const newHand = aiHand.filter((_, idx) => idx !== handIdx);
    const newBuilds = builds.filter((_, i) => i !== buildIdx);
    
    const currentCaptured = gameState.player2Captured || [];
    const newCaptured = [...currentCaptured, playedCard, ...build.cards];
    
    const nextTurn = 'player1';
    const msgText = `AI Opponent captured a build of ${build.value} with a ${formatCardForMessage(playedCard)}`;
    
    const updates = {
      player2Hand: newHand,
      builds: newBuilds,
      player2Captured: newCaptured,
      currentTurn: nextTurn,
      lastCapture: 'player2',
      lastPlayMessage: msgText,
      lastPlayTimestamp: Date.now()
  ...
  async function aiCaptureMultipleBuilds(handIdx, buildIndices) {
    const aiHand = gameState.player2Hand || [];
    const builds = gameState.builds || [];
    const playedCard = aiHand[handIdx];
    
    const selectedBuildsData = buildIndices.map(idx => builds[idx]);
    const allBuildCards = selectedBuildsData.flatMap(b => b.cards);
    ...
    const nextTurn = 'player1';
    const buildValue = selectedBuildsData[0].value;
    const msgText = `AI Opponent captured ${buildIndices.length} builds of ${buildValue} with a ${formatCardForMessage(playedCard)}`;
  ...
  async function aiCaptureTableCards(handIdx, tableIndices) {
    const aiHand = gameState.player2Hand || [];
    const tableCards = gameState.tableCards || [];
    const playedCard = aiHand[handIdx];
    
    const capturedTableCards = tableIndices.map(idx => tableCards[idx]);
    
    const newHand = aiHand.filter((_, idx) => idx !== handIdx);
    const newTableCards = tableCards.filter((_, idx) => !tableIndices.includes(idx));
    
    const currentCaptured = gameState.player2Captured || [];
    const newCaptured = [...currentCaptured, playedCard, ...capturedTableCards];
    
    const nextTurn = 'player1';
    const cardsStr = capturedTableCards.map(c => formatCardForMessage(c)).join(' + ');
    const msgText = `AI Opponent captured ${cardsStr} with a ${formatCardForMessage(playedCard)}`;
  ...
  async function executeAiBuild(move) {
    const aiHand = gameState.player2Hand || [];
    const tableCards = gameState.tableCards || [];
    const playedCard = aiHand[move.handIdx];
    
    const buildCards = [playedCard, ...move.tableIndices.map(idx => tableCards[idx])];
    
    const newHand = aiHand.filter((_, idx) => idx !== move.handIdx);
    const newTableCards = tableCards.filter((_, idx) => !move.tableIndices.includes(idx));
    
    const newBuild = {
      value: move.buildValue,
      cards: buildCards,
      owner: 'player2'
    };
    ...
    const nextTurn = 'player1';
    const msgText = `AI Opponent is building ${move.buildValue}s`;
  ...
  async function executeAiTrail(aiHand) {
    ...
    let lowestIdx = 0;
    let lowestRank = aiHand[0].rank;
    
    for (let i = 1; i < aiHand.length; i++) {
      if (aiHand[i].rank < lowestRank) {
        lowestRank = aiHand[i].rank;
        lowestIdx = i;
      }
    }
    
    const playedCard = aiHand[lowestIdx];
    const newHand = aiHand.filter((_, idx) => idx !== lowestIdx);
    
    const currentTableCards = gameState.tableCards || [];
    const newTableCards = [...currentTableCards, playedCard];
    
    const nextTurn = 'player1';
    const msgText = `AI Opponent trailed a ${formatCardForMessage(playedCard)}`;
